...
import com.example.testscoreandstatistics.databinding.DialogSelectionBinding
...
class MarksheetSelectionDialogFragment : DialogFragment() {

    private var _binding: DialogSelectionBinding? = null
    private val binding get() = _binding!!
    private lateinit var viewModel: MainActivityViewModel

    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        _binding = DialogSelectionBinding.inflate(requireActivity().layoutInflater)
    ...
    private fun handleOkClick() {
        ...
        // Construct params in the specific order requested
        val params = LinkedHashMap<String, String>()
        params["sessionToken"] = sessionToken
        params["mainClass"] = binding.mainClassDropdown.text.toString()
        params["subclass"] = binding.subclassDropdown.text.toString()
    ...
    private fun setupListeners(dialog: AlertDialog) {
        binding.subjectDropdown.setOnItemClickListener { _, _, _, _ ->
            ...
            binding.subclassDropdown.text.clear()
            binding.mainClassDropdown.setAdapter(createAdapter(mainClasses))
        ...
        binding.mainClassDropdown.setOnItemClickListener { _, _, _, _ ->
            val selectedSubject = binding.subjectDropdown.text.toString()
            val selectedMainClass = binding.mainClassDropdown.text.toString()
            val subclasses = viewModel.getSubclassesForMainClass(selectedSubject, selectedMainClass)
        ...
        binding.subclassDropdown.setOnItemClickListener { _, _, _, _ ->
            updateOkButtonState(dialog)
        }
    ...
    private fun updateOkButtonState(dialog: AlertDialog?) {
        val isAllSelected = binding.subjectDropdown.text.isNotEmpty() &&
                ...
                binding.subclassDropdown.text.isNotEmpty() &&
    ...
    companion object {
        const val TAG = "MarksheetSelectionDialogFragment"
    }
